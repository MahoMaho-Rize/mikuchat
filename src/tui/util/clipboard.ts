...
export namespace Clipboard {
  ...
  export async function read(): Promise<Content | undefined> {
    ...
    if (os === "darwin") {
      const tmpfile = path.join(tmpdir(), "mikuchat-clipboard.png");
      try {
        await Process.run(
          [
            "osascript",
    ...
    if (os === "win32" || release().includes("WSL")) {
      ...
      const base64 = await Process.text(
    ...
    if (os === "linux") {
      ...
      if (wayland.stdout.byteLength > 0) {
        return {
          data: Buffer.from(wayland.stdout as any).toString("base64"),
          mime: "image/png",
        };
      }
      ...
      if (x11.stdout.byteLength > 0) {
        return {
          data: Buffer.from(x11.stdout as any).toString("base64"),
          mime: "image/png",
        };
      }
    }
  ...
  const getCopyMethod = lazy(() => {
    ...
    if (os === "linux") {
      if (process.env["WAYLAND_DISPLAY"] && which("wl-copy")) {
        ...
        return async (text: string) => {
          const proc = Process.spawn(["wl-copy"], {
            stdin: "pipe",
            stdout: "ignore",
            stderr: "ignore",
          });
