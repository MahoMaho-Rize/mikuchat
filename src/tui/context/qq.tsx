...
export const {
  ...
  init: () => {
    ...
    const [wsStatus, setWsStatus] = createSignal<QQClientStatus>("disconnected")
    const [napcatStatus, setNapCatStatus] = createSignal<NapCatStatus>("stopped")
    ...
    const unsubNapcat = napcat.onStatus((s, info) => {
      ...
      if (s === "running" && wsStatus() !== "connected" && wsStatus() !== "connecting") {
    ...
    return {
      ...
      get napcatStatus() { return napcatStatus() },
      get napcatInfo() { return napcatInfo() },
