...
export function Prompt(props: PromptProps) {
    ...
    return [
      ...
      },
      {
        ...
        onSelect: async (dialog) => {
          ...
          const nonTextParts = store.prompt.parts.filter(
            (p) => p.type !== "text",
          );
  ...
  async function pasteImage(file: {
    ...
    const currentOffset = input.visualCursor.offset;
    const extmarkStart = currentOffset;
    const count = store.prompt.parts.filter(
  ...
  const placeholderText = createMemo(() => {
    ...
    if (store.mode === "shell") {
      const example =
        SHELL_PLACEHOLDERS[store.placeholder % SHELL_PLACEHOLDERS.length];
                ...
                <box flexDirection="row" gap={1} flexShrink={0}>
                  {(() => {
                    ...
                    const message = createMemo(() => {
                      ...
                      if (!r) return;
                      if (
                        r.message.includes("exceeded your current quota") &&
                        r.message.includes("gemini")
                      )
