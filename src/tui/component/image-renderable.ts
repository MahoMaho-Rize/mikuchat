...
import { Renderable, type RenderableOptions, OptimizedBuffer } from "@opentui/core"
import fs from "fs"
...
function releaseFromPool(hash: string) {
  ...
  if (entry.refCount <= 0) {
    ...
    if (entry.transmitted) {
      try { fs.writeSync(1, `\x1b_Ga=d,d=I,i=${entry.kittyId},q=2\x1b\\`) } catch {}
    }
    pool.delete(hash)
  }
}
...
function transmitEntry(entry: PoolEntry) {
  ...
  try { fs.writeSync(1, out.join("")) } catch {}
...
export class ImageRenderable extends Renderable {
  ...
  private deletePlacement() {
    if (this._placed && this._entry) {
      try {
        fs.writeSync(1, `\x1b_Ga=d,d=i,i=${this._entry.kittyId},q=2\x1b\\`)
      } catch {}
      this._placed = false
      this._lastRect = ""
    }
